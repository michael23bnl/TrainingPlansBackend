...
public class PreparedPlansController : ControllerBase
{
    ...
    public PreparedPlansController(IPlansRepository plansRepository, 
    ...
    public async Task<ActionResult<Guid>> CreatePreparedPlan([FromBody] PlanRequest request)
    {
        ...
        var (plan, response) = PlanModel.Create(Guid.NewGuid(), request.name, exercises, null);
